// js/strap-customizer.js
// === SnapInk Strap Customizer (defensive version) ===

document.addEventListener("DOMContentLoaded", () => {
  const $ = (id) => document.getElementById(id);

  // ---- DOM refs (match customize.html) ----
  const bgImageEl   = $("bgImage");
  const strapRectEl = $("strapRect");
  const overlayEl   = $("strap-overlay");
  const textLayerEl = $("strapTextLayer");

  const strapText     = $("strapText");
  const fontSelect    = $("font");
  const sizeRange     = $("size");
  const trackingRange = $("tracking");
  const textColor     = $("textColor");
  const strapColor    = $("strapColor");
  const upperCheck    = $("upper");
  const shadowCheck   = $("shadow");
  const downloadBtn   = $("download");

  // Optional: background swap buttons
  const bgButtons = Array.from(document.querySelectorAll("[data-bg]"));

  // Canvas for export (optional)
  const canvas = $("exportCanvas");
  const ctx = canvas ? canvas.getContext("2d") : null;

  const CF_BG =
    "https://imagedelivery.net/fYqzQPf1CS3qAF1vZpfSLw/e0459dcc-97bf-444c-6f05-330f77204f00/public";

  // ---- State (initialize from inputs when present) ----
  const state = {
    bgSrc: CF_BG,
    strapColor: (strapColor && strapColor.value) || "#111111",
    text: (strapText && strapText.value) || "YOUR TEXT",
    font: (fontSelect && fontSelect.value) || "Inter, system-ui, sans-serif",
    size: (sizeRange && parseInt(sizeRange.value, 10)) || 30,
    tracking: (trackingRange && parseInt(trackingRange.value, 10)) || 1,
    textColor: (textColor && textColor.value) || "#ffffff",
    upper: !!(upperCheck && upperCheck.checked),
    shadow: (shadowCheck && shadowCheck.checked) !== false, // default true
  };

  // Helpful diagnostics once on load
  const requiredIds = {
    bgImage: !!bgImageEl,
    strapRect: !!strapRectEl,
    "strap-overlay": !!overlayEl,
    strapTextLayer: !!textLayerEl,
  };
  Object.entries(requiredIds).forEach(([id, ok]) => {
    if (!ok) console.warn(`[SnapInk] Missing element with id="#${id}"`);
  });

  // ---- Render preview (never touches .style on null) ----
  function applyPreview() {
    if (bgImageEl && state.bgSrc && bgImageEl.getAttribute("src") !== state.bgSrc) {
      bgImageEl.setAttribute("src", state.bgSrc);
    }
    if (strapRectEl) {
      strapRectEl.style.background = state.strapColor;
    }
    if (textLayerEl) {
      const content = state.upper
        ? (state.text || "").toUpperCase()
        : (state.text || "");
      textLayerEl.textContent = content || "YOUR TEXT";

      textLayerEl.style.fontFamily = state.font;
      textLayerEl.style.fontSize = `${state.size}px`;
      textLayerEl.style.letterSpacing = `${state.tracking}px`;
      textLayerEl.style.color = state.textColor;
      textLayerEl.style.textShadow = state.shadow
        ? "0 1px 2px rgba(0,0,0,.55)"
        : "none";
    }
  }

  // ---- Event wiring (only if control exists) ----
  bgButtons.forEach((btn) => {
    btn.addEventListener("click", () => {
      if (!btn.dataset.bg) return;
      state.bgSrc = btn.dataset.bg;
      bgButtons.forEach((b) =>
        b.setAttribute("aria-pressed", b === btn ? "true" : "false")
      );
      applyPreview();
    });
  });

  if (strapText) {
    strapText.addEventListener("input", (e) => {
      state.text = e.target.value;
      applyPreview();
    });
  }

  if (fontSelect) {
    fontSelect.addEventListener("change", (e) => {
      state.font = e.target.value;
      applyPreview();
    });
  }

  if (sizeRange) {
    sizeRange.addEventListener("input", (e) => {
      state.size = parseInt(e.target.value, 10) || 30;
      applyPreview();
    });
  }

  if (trackingRange) {
    trackingRange.addEventListener("input", (e) => {
      state.tracking = parseInt(e.target.value, 10) || 0;
      applyPreview();
    });
  }

  if (textColor) {
    textColor.addEventListener("input", (e) => {
      state.textColor = e.target.value || "#ffffff";
      applyPreview();
    });
  }

  if (strapColor) {
    strapColor.addEventListener("input", (e) => {
      state.strapColor = e.target.value || "#111111";
      applyPreview();
    });
  }

  if (upperCheck) {
    upperCheck.addEventListener("change", (e) => {
      state.upper = !!e.target.checked;
      applyPreview();
    });
  }

  if (shadowCheck) {
    shadowCheck.addEventListener("change", (e) => {
      state.shadow = !!e.target.checked;
      applyPreview();
    });
  }

  // ---- Export (optional) ----
  if (downloadBtn && canvas && ctx) {
    downloadBtn.addEventListener("click", async () => {
      try {
        await exportPNG();
      } catch (err) {
        console.error("[SnapInk] Export failed:", err);
        alert("Could not export PNG. See console for details.");
      }
    });
  }

  // Initial paint
  applyPreview();

  // ===== Helpers =====
  function loadImage(src) {
    return new Promise((resolve, reject) => {
      const i = new Image();
      i.crossOrigin = "anonymous";
      i.onload = () => resolve(i);
      i.onerror = reject;
      i.src = src;
    });
  }

  function drawObjectCover(ctx, img, cw, ch) {
    const iw = img.width,
      ih = img.height;
    const scale = Math.max(cw / iw, ch / ih);
    const w = iw * scale;
    const h = ih * scale;
    const x = (cw - w) / 2;
    const y = (ch - h) / 2;
    ctx.drawImage(img, x, y, w, h);
  }

  function roundRect(ctx, x, y, w, h, r) {
    const rr = Math.min(r, w / 2, h / 2);
    ctx.beginPath();
    ctx.moveTo(x + rr, y);
    ctx.arcTo(x + w, y, x + w, y + h, rr);
    ctx.arcTo(x + w, y + h, x, y + h, rr);
    ctx.arcTo(x, y + h, x, y, rr);
    ctx.arcTo(x, y, x + w, y, rr);
    ctx.closePath();
  }

  function cssFontToCanvasFont(cssStack) {
    const first = cssStack.split(",")[0].trim();
    return first.replace(/['"]/g, "") || "Inter";
  }

  function drawSpacedText(ctx, text, xCenter, y, spacing) {
    if (!text) return;
    const widths = [...text].map((ch) => ctx.measureText(ch).width);
    const totalSpacing = spacing * Math.max(0, text.length - 1);
    const totalWidth = widths.reduce((a, b) => a + b, 0) + totalSpacing;

    let x = xCenter - totalWidth / 2;
    for (let i = 0; i < text.length; i++) {
      const ch = text[i];
      ctx.fillText(ch, x + widths[i] / 2, y);
      x += widths[i] + spacing;
    }
  }

  async function exportPNG() {
    const W = canvas.width || 1600;
    const H = canvas.height || 1000;
    ctx.clearRect(0, 0, W, H);

    // 1) BG
    const bg = await loadImage(state.bgSrc);
    drawObjectCover(ctx, bg, W, H);

    // 2) Strap rect (mirror CSS %)
    const strap = {
      x: 0.15 * W,
      y: 0.72 * H,
      w: 0.7 * W,
      h: 0.115 * H,
    };
    roundRect(ctx, strap.x, strap.y, strap.w, strap.h, 16);
    ctx.fillStyle = state.strapColor;
    ctx.fill();

    // 3) Sheen
    const grd = ctx.createLinearGradient(0, strap.y, 0, strap.y + strap.h);
    grd.addColorStop(0, "rgba(255,255,255,0.10)");
    grd.addColorStop(1, "rgba(0,0,0,0.35)");
    roundRect(ctx, strap.x, strap.y, strap.w, strap.h, 16);
    ctx.fillStyle = grd;
    ctx.fill();

    // 4) Text
    const content = state.upper
      ? (state.text || "").toUpperCase()
      : state.text || "YOUR TEXT";
    const fontPx = Math.max(10, Math.min(96, state.size * (W / 800)));
    ctx.font = `700 ${fontPx}px ${cssFontToCanvasFont(state.font)}`;
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillStyle = state.textColor;

    if (state.shadow) {
      ctx.shadowColor = "rgba(0,0,0,0.55)";
      ctx.shadowBlur = 6;
      ctx.shadowOffsetY = 2;
    } else {
      ctx.shadowColor = "transparent";
      ctx.shadowBlur = 0;
      ctx.shadowOffsetY = 0;
    }

    const spacing = state.tracking * (W / 800);
    drawSpacedText(ctx, content, strap.x + strap.w / 2, strap.y + strap.h / 2, spacing);

    // 5) Save
    const url = canvas.toDataURL("image/png");
    const a = document.createElement("a");
    a.download = "snapink-strap.png";
    a.href = url;
    a.click();
  }
});
